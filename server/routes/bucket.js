...
router.post('/:itemId/data', authAllowUser, function (req, res) {
  ...
  payload.___t = Date.now();

  var
    itemId = req.params.itemId,
    userId = ras.resourceOwnerId(req),
    updateFields = { $push: {data: payload} }
    ;

  // TODO: simplify to just one call
  ...
  bucketService.findOne(itemId, userId, function (err, item) {
    ...
    if( !item ){
      ...
      bucketService.updateOne(itemId, updateFields, function (err, item) {
        if (err != null) {
          return res.status(400).send({errors: [err]});
        }
...
router.delete('/:itemId/data/:timestamp', authAllowUser, function (req, res) {
  ...
  var
    ...
    userId = ras.resourceOwnerId(req),
    updateFields = { $pull: {data: { ___t: timestamp }} }
    ;
